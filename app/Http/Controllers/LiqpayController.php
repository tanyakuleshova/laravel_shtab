...

{
    ...
    public function createForm($text_to = '') {
    ...
    $fields = [
        'version' => '3',
        'action'  => 'data',
        'info' => $text_to?$text_to:'Запись №'.$order_id,
        'description' =>  $text_to,
            'result_url' => route('callback'), // перенаправление клиента на страницу 'result_url', после оплаты
//        'result_url' => "https://shtab.laracode.space/join-us",
        'server_url' => route('callback'), // callback принимающий ответ от liqpay
        'order_id'    => $order_id,
//            'orderDate'         => \Carbon::now(),
        'amount'            => request('benefactor_sum'),
        'currency'          => 'UAH',
        'productName'       => $text_to?$text_to:'Запись №'.$order_id,
        'productPrice'      => request('benefactor_sum'),
        'productCount'      => 1,
        'clientEmail'       => request('benefactor_sum')
    ];
    ...
    public function cnb_form($params)
    {
        ...
        return sprintf('
            <form method="POST" action="%s" accept-charset="utf-8">
                %s
                %s
                <input id="form_for_pay" type="image" src="//static.liqpay.ua/buttons/p1%s.radius.png" name="btn_text" />
            </form>
            ',
            $this->_checkout_url,
